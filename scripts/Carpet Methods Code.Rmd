...
mo_mass_small <- mo_mass %>% 
  ...
  dplyr::summarize(mean_1seed = mean(lot_mass_g)/25,
                   sd_1seed = sd(lot_mass_g)/25)

mo_mass_small # missouri seed mass mean and sd
...
co_mass_small # colorado seed mass mean and sd
...
mo_raw <- ggplot(mo_raw, aes(x = time_month, y = pct_recovered, fill = treatment))+
...
co_raw <- ggplot(co_raw, aes(x = time_month, y = pct_recovered, fill = treatment))+
  geom_boxplot(alpha = 0.5)+
  facet_grid(cols = vars(species))+
  scale_fill_manual(values = palatte, name = "Trap Type")+
  theme_bw()+
